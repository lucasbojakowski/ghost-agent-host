...
impl ScriptingBridge {
    ...
    pub fn call(&self, module: &str, function: &str, args: Vec<Value>) -> Result<Value> {
        Ok(self.adapter.call(module, function, args)?)
    }
...
fn run_scripting_probe(scripting: &FlScriptingAdapter) -> Result<ScriptingProbe> {
    ...
    observations.extend([
        ...
        probe(scripting, "songPosition", "transport", "getSongPos", vec![]),
        probe(
            scripting,
            "songPositionHint",
            "transport",
        ...
        probe(scripting, "loopMode", "transport", "getLoopMode", vec![]),
        probe(scripting, "isPlaying", "transport", "isPlaying", vec![]),
    ]);
