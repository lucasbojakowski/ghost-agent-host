...
impl eframe::App for GhostLab {
    fn update(&mut self, context: &egui::Context, _frame: &mut eframe::Frame) {
        egui::CentralPanel::default().show(context, |ui| {
            ui.heading("Ghost Agent Host Lab");
            // ui.label("Standalone validation surface for analysis, prompt compilation and mock plugin processing.");
            ui.separator();
            ui.horizontal(|ui| {
                ui.label("WAV file");
...
fn main() -> eframe::Result<()> {
    eframe::run_native(
        ...
        Box::new(|_| Ok(Box::new(GhostLab::default()))),
    )
}
