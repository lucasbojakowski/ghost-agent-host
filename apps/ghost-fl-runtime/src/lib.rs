...
const LEGACY_INDEX_HTML: &str = include_str!("../web/index.html");
const MAX_HTTP_BYTES: usize = 2 * 1024 * 1024;
const MAX_RECENT_EVENTS: usize = 500;

#[derive(RustEmbed)]
#[folder = "../../web/runtime/build/"]
...
impl Runtime {
            ...
            match listener.accept() {
                Ok((mut stream, _)) => {
                    ...
                    let is_websocket = stream
                        .peek(&mut preview)
                        .ok()
                        .and_then(|count| std::str::from_utf8(&preview[..count]).ok())
                        .is_some_and(|headers| headers.starts_with("GET /api/ws "));
                    if is_websocket {
                        let runtime = self.clone();
                        thread::Builder::new()
                            .name("ghost-fl-runtime-websocket".into())
                            .spawn(move || {
                                if let Err(error) = runtime.handle_websocket(stream) {
                                    eprintln!("[ghost-fl-runtime] WebSocket closed: {error:#}");
                                }
                            })
                            .context("failed to spawn WebSocket connection")?;
                        continue;
                    }
                    let request = (|| -> Result<()> {
                        stream.set_nonblocking(false)?;
    ...
    fn handle_websocket(&self, stream: TcpStream) -> Result<()> {
        stream.set_write_timeout(Some(Duration::from_secs(2)))?;
        let mut socket = tungstenite::accept(stream).context("WebSocket handshake failed")?;
        let events = self.journal.recent()?;
        let mut last_sequence = events.last().map_or(0, |event| event.sequence);
        let snapshot = json!({
            "kind": "snapshot",
            "state": self.state()?.clone(),
            "events": events,
        });
        socket.send(Message::Text(serde_json::to_string(&snapshot)?.into()))?;
        ...
        let _ignored = socket.close(None);
        Ok(())
    }
    ...
    fn handle_http(&self, stream: &mut TcpStream) -> Result<()> {
        ...
        match (request.method.as_str(), request.path.as_str()) {
            ...
            ("GET", "/api/apps") => {
                let state = self.state()?.clone();
                let apps = if self.cli.no_app {
                    Vec::new()
                } else {
                    let spec = self.cli.app.spec();
                    vec![RegisteredApp {
                        id: state.app.profile.clone(),
                        display_name: spec.display_name.to_owned(),
                        route: format!("/apps/{}", state.app.profile),
                        ui_url: format!("http://{}", state.app.endpoint),
                        endpoint: state.app.endpoint.clone(),
                        healthy: state.app.healthy,
                    }]
                };
                send_json(stream, "200 OK", &apps)
            }
            ("POST", "/api/app/start") => {
                self.start_app()?;
                self.wait_for_app_ready()?;
            ...
            ("GET", path) if !path.starts_with("/api/") => serve_web_asset(stream, path),
...
fn serve_web_asset(stream: &mut TcpStream, request_path: &str) -> Result<()> {
    ...
    if let Some(asset) = RuntimeWebAssets::get(candidate) {
        let content_type = mime_guess::from_path(candidate)
            .first_or_octet_stream()
            .essence_str()
            .to_owned();
        let cache_control = if candidate.starts_with("_app/immutable/") {
    ...
    if !candidate
        ...
        .contains('.')
    {
        if let Some(index) = RuntimeWebAssets::get("index.html") {
            return send_asset_response(
                stream,
                "200 OK",
